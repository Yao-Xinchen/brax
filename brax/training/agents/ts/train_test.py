...
from brax.training.agents.ts import networks_vision as ts_networks_vision
from brax.training.agents.ts import train as ts
import jax


class TSTest(parameterized.TestCase):
  """Tests for TS module."""


  @parameterized.parameters('ndarray', 'dict_state')
  def testTrain(self, obs_mode):
    """Test TS with a simple env."""
    fast = envs.get_environment('fast', obs_mode=obs_mode)
    _, _, metrics = ts.train(
        fast,
        num_timesteps=2**15,
        episode_length=128,
        num_envs=64,
  ...
  def testTrainWithNetworkParams(self, distribution_type, noise_std_type):
    """Test TS runs with different network params."""
  ...
  def testTrainAsymmetricActorCritic(self):
    """Test TS with asymmetric actor critic."""
    ...
    _, (_, policy_params, value_params), _ = ts.train(
        env,
        num_timesteps=2**15,
        episode_length=1000,
        num_envs=64,
  ...
  def testNetworkEncoding(self, normalize_observations):
    ...
    original_inference, params, _ = ts.train(
        env,
        num_timesteps=128,
        episode_length=128,
        num_envs=128,
    ...
    ts_network = ts_networks.make_ts_networks(
        env.observation_size, env.action_size, normalize_fn
    )
    inference = ts_networks.make_inference_fn(ts_network)
  ...
  def testTrainDomainRandomize(self):
    """Test TS with domain randomization."""
  ...
  def testPixelsTS(self, asymmetric_obs, obs_mode):
    """Test TS with pixel observations."""
    env = envs.get_environment(
        'fast',
        pixel_obs=True,
    ...
    network_factory = functools.partial(
        ts_networks_vision.make_ts_networks_vision,
        policy_hidden_layer_sizes=(32,),
        value_hidden_layer_sizes=(32,),
