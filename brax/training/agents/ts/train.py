...
from brax.training.agents.ts import checkpoint
from brax.training.agents.ts import losses as ts_losses
from brax.training.agents.ts import networks as ts_networks
...
class TrainingState:
  ...
  params: ts_losses.TSNetworkParams
...
def train(
    ...
    network_factory: types.NetworkFactory[
        ts_networks.TSNetworks
    ] = ts_networks.make_ts_networks,
    seed: int = 0,
    # eval
    num_evals: int = 1,
    eval_env: Optional[envs.Env] = None,
  ...
  Args:
    ...
    randomization_fn: a user-defined callback function that generates randomized
      environments
    learning_rate: learning rate for ts loss
    entropy_cost: entropy reward for ts loss, higher values increase entropy of
      the policy
    discounting: discounting rate
    unroll_length: the number of timesteps to unroll in each environment. The
      TS loss is computed over `unroll_length` timesteps
    ...
    clipping_epsilon: clipping epsilon for TS loss
    ...
    restore_params: raw network parameters to restore the TrainingState from.
      ...
      from the return values of ts.train().
  ...
  ts_network = network_factory(
      obs_shape, env.action_size, preprocess_observations_fn=normalize
  )
  make_policy = ts_networks.make_inference_fn(ts_network)
  ...
  init_params = ts_losses.TSNetworkParams(
      policy=ts_network.policy_network.init(key_policy),
      value=ts_network.value_network.init(key_value),
  )
